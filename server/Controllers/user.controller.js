...
async function signUp(req, res) {
    try {
        ...
        const query = "INSERT INTO users (phone,fullName, `password`, address) VALUES(?,?, ?,?);"
        let conn = await db.getConnection();
        conn = await db.beginTransaction(conn);
        const result = await db.queryTransaction(conn, query, [phone, fullName, bcrypt.hashSync(password, 10), address]);
...
async function login(req, res) {
    try {
        ...
        const query = 'SELECT phone,userId, `password`, role FROM `users` WHERE phone = ?'
        const result = await db.exeQuery(query, [phone]);
        if (!result.length) {
            res.json({
                statusCode: 404,
